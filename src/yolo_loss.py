...
class YOLOLoss(nn.Module):
    ...
    def split_prediction_tensor(self, prediction_tensor):
        ...
        box_vals = self.boxes_per_cell*5

        predicted_class_scores = prediction_tensor[..., box_vals:]
        predicted_boxes = prediction_tensor[..., :box_vals].reshape(batch_size, self.grid_size, self.grid_size, self.boxes_per_cell, 5)
    ...
    def xywh_to_xyxy(self, boxes_xywh, col_row):
        ...
        x_offset = boxes_xywh[:, 0:1]
        y_offset = boxes_xywh[:, 1:2]
        width = boxes_xywh[:, 2:3]
        height = boxes_xywh[:, 3:4]
        col_ind = col_row[:, 0:1]
        row_ind = col_row[:, 1:2]

        x_center = (col_ind + x_offset)/self.grid_size
        y_center = (row_ind + y_offset)/self.grid_size

        top_x = x_center - width/2
        top_y = y_center - height/2
        bot_x = x_center + width/2
        bot_y = y_center + height/2

        ans = torch.cat([top_x, top_y, bot_x, bot_y], dim=1)
        return ans
    ...
    def choose_responsible_box(self, predicted_boxes, target_boxes, has_object_mask):
        ...
        predicted_boxes = predicted_boxes[has_object_mask]
        target_boxes = target_boxes[has_object_mask]

        indices = has_object_mask.nonzero()
        col_row = indices[:, [2, 1]]
        ...
        if predicted_boxes.shape[0] == 0:
            return (torch.zeros(0,5, dtype=predicted_boxes.dtype, device=device), torch.zeros(0,1, dtype=predicted_boxes.dtype, device=device), torch.zeros(0, dtype=torch.long, device=device))
        ...
        num_object_cells = target_boxes.shape[0]
        responsible_box_predictions = torch.zeros((num_object_cells, 5), device=device)
        responsible_box_ious = torch.zeros((num_object_cells, 1), device=device)
        responsible_predictor_index = torch.zeros(num_object_cells, dtype=torch.long, device=device)

        for i in range(num_object_cells):
            pred = predicted_boxes[i]
            coord = col_row[i]

            with torch.no_grad():
                target_box = target_boxes[i]
                target_box = self.xywh_to_xyxy(target_box.unsqueeze(0), coord.unsqueeze(0))
                preds = self.xywh_to_xyxy(pred[:, :-1], torch.stack([coord] * self.boxes_per_cell))

                ious = compute_iou(preds, target_box)[:,0]

            responsible_box_ious[i][0], responsible_predictor_index[i] = ious.max(dim=0)
            responsible_box_predictions[i] = pred[responsible_predictor_index[i]]
    ...
    def build_responsible_mask(
        ...
        N = has_object_mask.shape[0]
        ans = torch.zeros((N, self.grid_size, self.grid_size, self.boxes_per_cell),
                          dtype=torch.bool, device = has_object_mask.device)

        if responsible_predictor_index.shape[0] == 0:
            return ans

        indices = has_object_mask.nonzero()

        ans[indices[:, 0], indices[:, 1], indices[:, 2], responsible_predictor_index] = True
    ...
    def regression_xy_loss(
        ...
        """
        vector = ((responsible_box_predictions[:,0] - target_boxes_for_object_cells[:,0]) ** 2) + ((responsible_box_predictions[:,1] - target_boxes_for_object_cells[:,1]) ** 2)
        ans = torch.sum(vector)
        return ans
    ...
    def regression_wh_loss(
        ...
        pred_w = responsible_box_predictions[:, 2].clamp(min=1e-6).sqrt()
        pred_h = responsible_box_predictions[:, 3].clamp(min=1e-6).sqrt()
        targ_w = target_boxes_for_object_cells[:, 2].clamp(min=1e-6).sqrt()
        targ_h = target_boxes_for_object_cells[:, 3].clamp(min=1e-6).sqrt()

        vector = (pred_w - targ_w) ** 2 + (pred_h - targ_h) ** 2
        ans = torch.sum(vector)
        return ans
    ...
    def object_confidence_loss(self, responsible_box_predictions, responsible_box_ious):
        ...
        target = responsible_box_ious[:, 0].detach()
        vector = (target - responsible_box_predictions[:,-1])**2
        ans = torch.sum(vector)
        return ans
    ...
    def no_object_confidence_loss(self, predicted_boxes, responsible_mask):
        ...
        """
        non_mask = ~responsible_mask

        boxes = predicted_boxes[non_mask]
        vector = boxes[:,-1]**2
        ans = torch.sum(vector)
        return ans
    ...
    def class_probability_loss(
        ...
        """
        preds = predicted_class_scores[has_object_mask]
        targets = target_class_scores[has_object_mask]

        vector = (preds - targets) ** 2
        ans = torch.sum(vector)
        return ans
    ...
    def forward(self, pred_tensor, target_boxes, target_cls, has_object_map):
        ...
        responsible_box_predictions, responsible_box_ious, responsible_predictor_index = self.choose_responsible_box(predicted_boxes, target_boxes, has_object_map)
        mask = self.build_responsible_mask(predicted_boxes, has_object_map, responsible_predictor_index)

        # TODO: Step 3: Compute regression loss terms.
        targets_for_objects = target_boxes[has_object_map]

        xy_loss = self.regression_xy_loss(responsible_box_predictions, targets_for_objects)
        wh_loss = self.regression_wh_loss(responsible_box_predictions, targets_for_objects)
        # TODO: Step 4: Compute confidence loss terms for object and no-object predictors.
        object_confidence = self.object_confidence_loss(responsible_box_predictions, responsible_box_ious)
        no_object_confidence = self.no_object_confidence_loss(predicted_boxes, mask)
        ...
        xy_loss = xy_loss / N
        wh_loss = wh_loss / N
        object_confidence = object_confidence / N
        no_object_confidence = no_object_confidence / N
        class_loss = class_loss / N

        loss = (xy_loss + wh_loss) * self.lambda_coord + object_confidence + no_object_confidence * self.lambda_noobj + class_loss
        ...
        return {
            ...
            "reg_xy_loss": xy_loss,
            "reg_wh_loss": wh_loss,
            "obj_loss": object_confidence,
            "no_obj_loss": no_object_confidence,
            "cls_loss": class_loss,
        }
