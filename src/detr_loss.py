...
class DETRSetCriterion(nn.Module):
    ...
    def loss_labels(self, outputs, targets, indices):
        ...
        ground_truths = torch.cat(
            [dictionary["labels"][ind] for dictionary, (_, ind) in zip(targets, indices)]
        )
        ...
        target_classes[idx] = ground_truths
    ...
    def loss_boxes(self, outputs, targets, indices, num_boxes):
        ...
        target_boxes = torch.cat(
            [dictionary["boxes"][ind] for dictionary, (_, ind) in zip(targets, indices)], dim=0
        )

        # TODO: Compute the L1 box loss.
        loss = F.l1_loss(boxes, target_boxes, reduction="none")
        ...
        giou_loss = (1-torch.diag(giou)).sum() / num_boxes

        return {"loss_bbox": loss, "loss_giou": giou_loss}
