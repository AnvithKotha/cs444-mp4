...
class SimpleDETR(nn.Module):
    ...
    def forward(
        ...
        processed_src = src.permute(0, 2, 3, 1).flatten(1,2)
        processed_pos = pos.permute(0, 2, 3, 1).flatten(1,2)
        processed_mask = mask.flatten(1)

        # TODO: Expand query_embed to the batch dimension so every image gets the same
        # set of learned queries.
        query_embedded = self.query_embed.weight.unsqueeze(0).repeat(src.shape[0], 1, 1)
        ...
        heads = self.transformer(
            processed_src + processed_pos,
            query_embedded,
            src_key_padding_mask=processed_mask,
            memory_key_padding_mask=processed_mask,
        )

        # TODO: Project the decoder output to class logits.
        outputs_classes = self.class_embed(heads)
        ...
        outputs_coord = self.bbox_embed(heads).sigmoid()
